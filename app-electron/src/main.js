...
ipcMain.handle("get-profile-seguro", async () => {
  try {
    // Ajusta la ruta si tu session.js está en otro lugar
    const session = require("../src/backend/session/session.js");
    const profile = await session.getProfile();
    return profile;
  } catch (error) {
    console.error("Error al obtener el perfil seguro:", error);
    return null;
  }
});
